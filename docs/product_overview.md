...
## Current State (as of 2026-03-27, updated after Session 25)
...
- **`external_link` and `downloadable_file` block types**: available in the `BlockEditor` add-block row for all editable lesson steps (not only hands-on steps). `external_link` stores the URL in `content` and the link label in `title`; renders as a clickable link card in view mode. `downloadable_file` stores a filename label in `title` and a description in `content`; renders as a file card with a "coming soon" placeholder for the actual upload.
